...
class TC_MessageConverters < Test::Unit::TestCase
 ...
 # !> `*' interpreted as argument prefix
  ...
  end # !> instance variable @timelines not initialized

  def test_expand
    UserConfig.stubs(:[]).with(:bitly_user).returns(nil)
    UserConfig.stubs(:[]).with(:bitly_apikey).returns(nil) # !> statement not reached
    UserConfig.stubs(:[]).with(:proxy_enabled).returns(nil) # !> redefine get_active_mumbles

    w = MessageConverters.expand_url_all('watching: http://mikutter.d.hachune.net/')
    assert_equal("watching: http://mikutter.d.hachune.net/", w) # !> method redefined; discarding old inspect
...
# ~> notice: ./addon/bitly/bitly.rb:82:in `expand_url_many': http://bit.ly/90tFUh
# ~> notice: ./addon/bitly/bitly.rb:82:in `expand_url_many': http://bit.ly/bkUjey
# ~> notice: ./addon/bitly/bitly.rb:82:in `expand_url_many': http://bit.ly/gsU1Ic
# >> Loaded suite -
# >> Started
# >> ..
# >> Finished in 1.321846 seconds.
# >> 
