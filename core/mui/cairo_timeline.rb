    ...
    Reserver.new(60) {
      Delayer.new {
        if !@tl.destroyed?
          ...
          @tl.correct_counter -= 1 if not window_active
          refresh if not(InnerTL.current_tl == @tl and window_active and Plugin.filtering(:get_idle_time, nil).first < 3600) and @tl.correct_counter <= (window_active ? -HYDE : 0)
